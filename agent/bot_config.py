...
LLAMA_CPP_URL: str = CFG.get("llama_cpp", {}).get("url", "http://localhost:8080")
MODEL_BACKEND: str = CFG.get("agent", {}).get("model", "ollama")  # ollama / llama-cpp
...
KNOWLEDGE_DIR: Path = Path(__file__).parent.parent / CFG.get("agent", {}).get("knowledge_dir", "knowledge")
