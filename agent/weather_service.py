...
DB_PATH = _base_dir / "data" / "search_cache.db"
...
def parse_weather(items):
    ...
    weather_desc = sky
    if pty != "0": weather_desc += f" (강수: {pty_str})"

    res = (
        f"📍 날씨 정보 (기상청 공식 데이터)\n"
        f"- **현재 기온: {cur_tmp}°C** 🌡️\n"
        f"- 하늘 상태: {weather_desc}\n"
        f"- 오늘 기온: 최고 {tmx}°C / 최저 {tmn}°C\n"
        f"※ 갱신 시각: {now_dt.strftime('%m/%d %H:%M')}"
    )
    return res
...
def fetch_air_quality(station_name: str):
    ...
    try:
        ...
        return (
            f"🌫️ 대기질 정보 (측정소: {station_name})\n"
            f"- 미세먼지(PM10): {pm10}㎍/㎥ ({pm10_grade})\n"
            f"- 초미세먼지(PM2.5): {pm25}㎍/㎥ ({pm25_grade})\n"
            f"- 오존(O₃): {o3} ppm\n"
            f"- 측정시각: {data_time}"
        )
    except Exception as e:
